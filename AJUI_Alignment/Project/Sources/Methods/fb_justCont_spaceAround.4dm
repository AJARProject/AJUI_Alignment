			...
			Case of 
				: ($flexbox.direction="row-reverse")  // Direction = Row Reverse
					...
					If ($coord.x>=$mainCoord.x) & ($current_line#$previous_line)
						...
						$count:=$itemsNewCoord.countValues($coord.line;"line")-1
						$spaceAround:=$sizeRest/(($count+1)*2)
					End if 
					
					If ($calc=True:C214) & ($spaceAround#0)
						$delta_width:=($spaceAround*($count*2))+$spaceAround  // Store the delta of the space left
				...
				: ($flexbox.direction="column")  // Direction = Column
					...
					If ($calc=True:C214) & ($spaceAround#0)
						$delta_height:=($spaceAround*($count*2))+$spaceAround  // Store the delta of the space left
				...
				: ($flexbox.direction="column-reverse")  // Direction = Column Reverse
					...
					$current_line:=$coord.line($coord.right+$coord.marginRight)
